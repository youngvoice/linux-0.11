...
ne2k_init()
{
		...
		outb(NE_DCR_WTS&(~NE_DCR_BOS), ne2k.iobase + NE_P0_DCR);
...
int ne2k_transmit(struct pbuf *p)
{
		...
		*/
		outb_p(ne2k.iobase + NE_P0_TPSR, (unsigned char)dst);
		if (packetlen > 60) {
				outb_p(ne2k.iobase + NE_P0_TBCR0, packetlen);
				outb_p(ne2k.iobase + NE_P0_TBCR1, packetlen >> 8);
		}
		else {
				outb_p(ne2k.iobase + NE_P0_TBCR0, 60);
				outb_p(ne2k.iobase + NE_P0_TBCR1, 0);
		}
		outb_p(ne2k.iobase + NE_P0_CR, NE_CR_RD2 | NE_CR_TXP | NE_CR_STA);
